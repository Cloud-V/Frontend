	...
	render() {
		...
		return (<FormGroup>
			...
			<FormGroup className="switch-toggle alert alert-light" inline>
				...
				<Button role="button" tag={Link} to={'#'} className="btn switch-button"></Button>
			</FormGroup>
			{touched && error && <div className="form-field-error">{error}</div>}
		</FormGroup>);
	}
...
class RepositoryForm extends Component {
	constructor(props) {
		super(props);
		if (props.editMode || props.cloneMode) {
		...
		return (
			<Form className="auth-form" onSubmit={handleSubmit}>
				...
				<div className="w-100 d-flex justify-content-between align-items-center">
					<Button type="submit" className="action-button" disabled={submitting}>{this.props.editMode? 'Save': (this.props.cloneMode? 'Clone': 'Create')}</Button>
				</div>
			</Form>
		);
	};
...
export default reduxForm({
	form: 'repositoryFrom', // a unique identifier for this form
	validate,
})(RepositoryForm);