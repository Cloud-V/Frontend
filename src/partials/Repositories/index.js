...
class Repositories extends Component {
	...
	onClone(repository) {
		this.props.setRepository(repository).then(action => {
			ReactGA.modalview(`/${repository.get('ownerName')}/${repository.get('repoName')}/clone`);
	...
	render() {
		...
		return (
			<React.Fragment>
				<Container className="h-100 repositories d-flex flex-column" fluid>
				<Row className="">
					{_.chunk(this.props.repositories.get('data').toArray(), 3).map((repos, index) => (<React.Fragment key={index}>
						{repos.map(repo => <Repository key={repo.get_id()} onClone={this.onClone} onDownload={this.props.onRepositoryDownload} repository={repo}/>)}
					</React.Fragment>))}
				</Row>
					{this.props.enablePagination?
						<Pagination
						className={`${this.props.repositories.get('data').size? '': 'd-none'}`}
						size={Math.ceil(pagination.count / PaginationData.ReposPerPage)}
						handlePageClick={this.props.handlePageClick}
						page={this.props.page}
						pageLink={this.props.pageLink || 'dashboard'}
						refreshPage={this.props.refreshPage} />: null}
				</Container>
				<CloneDialog
				...
				{this.props.hasCreateButton?
					<FloatButton onClick={this.openDialog} title="Create Repository" />
					: null
				}
			</React.Fragment>
		);
