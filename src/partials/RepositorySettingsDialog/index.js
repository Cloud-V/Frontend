...
class RepositorySettingsDialog extends Component {
	render() {
		if (this.props.modal && (this.props.status === Status.Success || this.props.status === Status.ActionSuccess)) {
			...
			if (ownerName !== this.props.ownerName || repoName !== this.props.repoName) {
				return <Redirect push to={`/${ownerName}/${repoName}`} />
			}
		}
		return (
			<Modal isOpen={this.props.modal} toggle={this.props.toggle} className="dialog repo-dialog">
				...
				<ModalBody className="modal-form">
					<div className="header d-flex justify-content-center align-items-center mb-5 mt-1">Repository Settings</div>
