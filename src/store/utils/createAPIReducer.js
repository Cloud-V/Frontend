...
export default (state, action, {
	...
	switch (action.type) {
		...
		case generateInProgressActionTypeName(actionName):
			return state
				.set('status', noLoading ? state.get('status') : (isAction ? Status.ActionLoading : Status.Loading))
				.set('error', '')
				.set('data', noResetOnStart ? state.get('data') : defaultData);
		case generateSuccessActionTypeName(actionName):
			return state
				.set('status', clearStatusOnSuccess ? Status.None : (isAction ? Status.ActionSuccess : Status.Success))
				.set('statusCode', action.status)
				.set('error', '')
				.set('data', parser(action.result));
		case generateErrorActionTypeName(actionName):
			return state
				.set('status', noError ? state.get('status') : (isAction ? Status.ActionError : Status.Error))
				.set('statusCode', action.status)
