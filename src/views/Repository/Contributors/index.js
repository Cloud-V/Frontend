...
class Contributors extends Component {
	...
	promptEdit(contr) {
		this.props.setContributor(contr).then(action => {
			...
			ReactGA.modalview(`/${repository.get('data').get('ownerName')}/${repository.get('data').get('repoName')}/authorization/edit`);
	...
	promptRemove(contr) {
		this.props.setContributor(contr).then(action => {
			...
			ReactGA.modalview(`/${repository.get('data').get('ownerName')}/${repository.get('data').get('repoName')}/authorization/delete`);
			this.setState({
				removeModal: true,
	...
	openDialog() {
		this.props.setContributor(null).then(action => {
			...
			ReactGA.modalview(`/${repository.get('data').get('ownerName')}/${repository.get('data').get('repoName')}/authorization/new`);
			this.setState({
				editMode: false,
				modal: true
			});
	...
	renderRows() {
		...
		return _.map(this.props.contributors.get('data').toJS(), (contr, ind) => {
			return (<tr key={contr._id}>
				<td className="col-4">
					<Link className="nav-link user-link" to={`/${contr.user.username}`}>@{contr.user.username}</Link>
				</td>
				<td className="col-4">{contr.role}</td>
				<td className="col-4">
					<Button disabled={!isOwner || contr.role === 'Owner'} className="repository-details-action" onClick={e => this.promptEdit(contr)}>
						<i className="icomoon icon-edit fa-2x"/>
						<span>Edit</span>
					</Button>
					<Button disabled={!isOwner || contr.role === 'Owner'} className="repository-details-action" onClick={e => this.promptRemove(contr)}>
	...
	render() {
		...
		return (<React.Fragment>
			...
			<ConfirmationDialog
						...
						disabled={this.props.contributor.get('status') === 'loading'}
						...
						onResponse={(confirmed) => {
							if (confirmed) {
								this.props.onContributorRemove(this.props.contributor.get('data')).then((action) => {
									this.closeDialog()
								}).catch(err => {
									this.setState({
										removeError: err
			...
			<Table className="repository-table mb-0" responsive>
				<thead>
					<tr>
						...
						<th className="repository-details-action-header col-4">Possible Actions</th>
					</tr>
				</thead>
				<tbody>
				{this.renderRows()}
			...
			{isOwner && <div className="action-button-wrapper pt-2">
				<Button className="action-button mt-5" onClick={this.openDialog}>Add User</Button>
			</div>}
		</React.Fragment>);
	}
}
