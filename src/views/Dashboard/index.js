...
import { markDashboard } from "store/actions/user";
...
const mapDispatchToProps = (dispatch) => {
    return {
        ...
        repositoriesGet: ({ ownerName }, page = 0) =>
            dispatch(
                getUserRepositories(
                    {
                        ownerName,
...
class Dashboard extends Component {
    constructor(props) {
        ...
        const paramsOwnerName = (
            props.match.params.ownerName || ""
        ).toLowerCase();
    ...
    tourHandler(data) {
        ...
        if (
            ...
            (action === "next" && index === 4 && status === "finished")
        ) {
        ...
        return (
            <div className="h-100">
                <Joyride
                    ...
                    run={
                        run &&
                        this.props.user.get("data").get("dashboardTour") ===
                            false
                    }
                ...
                >
                    ...
                    >
                        <Repositories
                            ...
                            refreshPage={(page) =>
                                this.refreshPage("repositories", page)
                            }
                            onRepositorySubmit={(repo) =>
                                this.props.createRepository(repo.toJS())
                            }
                            ...
                            onOpenDialog={(opened) =>
                                opened ? this.setState({ run: false }) : null
                            }
                        />
                    </TabPane>
                    <TabPane className="h-100" tab="Shared" key="shared">
                        <Repositories
                            ...
                            enablePagination={true}
                            handlePageClick={(page, offset) =>
                                this.handlePageClick("shared", page, offset)
                            }
                            page={this.state.sharedPage}
                            refreshPage={(page) =>
                                this.refreshPage("shared", page)
                            }
                            onRepositoryClone={(repo, target) =>
                                this.props.cloneRepository(
                                    repo.toJS(),
