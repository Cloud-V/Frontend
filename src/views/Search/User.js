...
class User extends Component {
	render() {
		...
		const displayName = userData.displayName || (userData.username.length ? ('@' + userData.username) : '');
			...
			<Card className="search-user">
				<CardBody className="d-flex">
					<div className="d-flex mt-2 mr-4 h-100 justify-content-center align-items-center">
						<Link to={`/${username}`}>
							<img src={avatarURL} className="img-avatar" alt={displayName}></img>
						</Link>
					</div>
					<div className="d-flex flex-column">
						...
						<div className="user-repositories">
							<Link className="profile-repositories-link plain-link black" to={`/${URLs.Repositories}/${username}`}>Repositories</Link>
						</div>
					</div>
				</CardBody>
			</Card>
