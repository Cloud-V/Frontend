...
class Search extends Component {
	constructor(props) {
		...
		this.handleRepositoriesPageClick = this.handleRepositoriesPageClick.bind(this);
	...
	fetchResults() {
		if (this.state.q.length) {
			...
			this.props.searchRepositories({
				...
				const repositoriesCount = repositoriesWrapper.get('pagination').get('count') || 0;
	...
	async componentDidUpdate() {
		const q = (queryString.parse(this.props.location.search).q || '').trim();
		if (q !== this.state.q) {
			this.setState({
				q
			}, () => {
	...
	render() {
		...
		if (search.get('status') === 'loading' || (search.get('status') === '' && hasQuery)) {
			...
			return (<div className="app">
					...
					<div className="flex-1 w-100 d-flex">
						<Alert className="h-10 flex-1" color="danger">{search.get('error')}</Alert>
					</div>
					<Footer />
				</div>);
		}
				...
				{hasQuery?
						<Row className="mr-0">
								...
								<Row className="search-explore-tab">
									<Button
										className={classnames({
											active: this.state.activeTab === 'repositories'
										})}
										onClick={() => this.toggle('repositories')}>
										Repositories
							...
							<Col xl="10" lg="9" className="pr-0">
								<TabContent activeTab={this.state.activeTab} className="search-explore-tab-content">
									<TabPane tabId="repositories" className="search-explore-tab-pane">
										<Row className="mr-0">
											{hasRepositories?
												(<React.Fragment>
													{_.chunk(repositories.toArray(), 3).map((repos, index) => (<React.Fragment key={index}>
													...
													<div className="w-100 d-flex justify-content-center mr-md-6 mr-sm-0">
														<Pagination
																className=""
																size={Math.ceil(repositoriesCount / PaginationData.ReposPerPage)}
																handlePageClick={this.handleRepositoriesPageClick}
																page={this.state.repositoriesPage}
																pageLink={`search?q=${this.state.q}`}
																refreshPage={this.refreshRepositoriesPage} />
													</div>
												</React.Fragment>)
												: (<div className="w-100 h-100 d-flex justify-content-center align-items-center">
														<i className="fa fa-search fa-4x mr-4"></i>
														<h3 className="d-flex justify-content-center align-items-center">We didn't find repositories matching your search term.</h3>
													</div>)
											}
										</Row>
									</TabPane>
									<TabPane tabId="users">
										<Row>
											{hasUsers?
												<React.Fragment>
													{_.chunk(users.toArray(), 3).map((users, index) => (<React.Fragment key={index}>
														{users.map(user => <User key={user.get_id()} user={user}/>)}
													</React.Fragment>))}
													<div className="w-100 d-flex justify-content-center mr-md-6 mr-sm-0">
														<Pagination
																className=""
																size={Math.ceil(usersCount / PaginationData.UsersPerPage)}
																handlePageClick={this.handleUsersPageClick}
																page={this.state.usersPage}
																pageLink={`search?q=${this.state.q}`}
																refreshPage={this.refreshUsersPage} />
													</div>
												</React.Fragment>
												: <div className="w-100 h-100 d-flex justify-content-center align-items-center">
														<i className="fa fa-search fa-4x mr-4"></i>
														<h3 className="d-flex justify-content-center align-items-center">We didn't find users matching your search term.</h3>
													</div>
											}
										</Row>
									</TabPane>
					...
					: (<div className="w-100 h-100 d-flex mt-20 justify-content-center align-items-center">
						...
						<h2 className="d-flex justify-content-center align-items-center">
							Use the above search box to search Cloud V repositories and users.
						</h2>
					</div>)
				}
				</Container>
