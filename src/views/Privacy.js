...
class Privacy extends Component {
	...
	render() {
		...
		if (privacy.get('status') === 'loading' || (privacy.get('status') === '')) {
			...
			return (<div className="app">
					...
					<div className="flex-1 w-100 d-flex">
						<Alert className="h-10 flex-1" color="danger">{privacy.get('error')}</Alert>
					</div>
					<Footer />
				</div>);
		}
		...
		return (
			<div className="app">
				<Navbar location={this.location}/>
				<Container className="privacy flex-1 w-100 h-100 pl-0 pr-0 d-flex flex-column justify-content-center align-items-center" fluid>
					<div className="w-100 h-100 d-flex flex-column justify-content-center align-items-center">
						<div className="privacy-section privacy-section-content w-75 mb-4 mt-4">
							<ReactMarkdown source={privacy.get('data').get('privacy')} />
						</div>
					</div>
				</Container>
				<Footer />
