...
class CompleteSignup extends Component {
	render() {
		...
		if (user.get('status') === 'action-loading') {
			return (<Container className="app h-100 d-flex justify-content-center align-items-center" fluid>
			<SquareSpinner />
			</Container>);
		}
		...
		return (
			<Container className="app h-100 d-flex justify-content-center align-items-center" fluid>
										...
										<CardBody className="profile-card w-100 d-flex flex-column justify-content-start align-items-center">
											<CompleteSignupForm
														...
														onSubmit={e => {
															...
															this.props.completeSignup({username});
														}} />
										</CardBody>
								</Card>)
						}
