	...
	render() {
		return (<div className="app">
				...
				{(() => {
					...
					const isLogged = !!(!user.get('data').isEmpty() &&
					...
					if (!this.hasCode) {
						return (<Alert color="danger">Google authentication failed.</Alert>)
					}
					if (user.get('status') === 'action-loading' || user.get('status') === '') {
						...
						return <Alert color="danger">{user.get('error').toString()}</Alert>
					}
					return null;
				})()}
				</Container>
