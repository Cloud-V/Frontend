...
class Workspace extends Component {
    constructor(props) {
        ...
        this.workspaceSettingsCallback =
            this.workspaceSettingsCallback.bind(this);
        ...
        if (matches.length) {
            ...
            return this.showPrompt({
                ...
                body: (
                    <React.Fragment>
                        {matches.length > 1
                            ? `${matches.length} entries with the same name already exist. Would you like to overwrite?`
                            : `An entry with the same name "${matches[0].title}" already exists. Would you like to overwrite?`}
                    </React.Fragment>
                ),
    ...
    onClose(selectedTreeNode, evt) {
        const file = (
            this.props.files
                .get("data")
                .find((el) => el.id === selectedTreeNode) || {
                toJS: () => null,
            }
        ).toJS();
        if (file) {
    ...
    async onSynthesize() {
        let stdcells = this.props.library.get("data").get("stdcells").toJS();

        if (stdcells.length < 1) {
            return this.toastError(
                "Cloud V has not been configured with standard cell libraries."
            );
        }
        await this.saveAll();
    ...
    simulationCallback({ level, simulationTime, fileName }, done) {
        ...
        if (targetNode.status === "loading") {
            return this.toastError(
                "Cannot start a new simulation while one is already loading."
            );
        }
    ...
    connectSockets() {
        ...
        socket.on("connect", () => {
            ...
            if (!token) {
                try {
                    token = JSON.parse(
                        Storage.get(StorageKeys.TrialUser)
                    ).token;
                    if (!token) {
                        return this.toastError(
