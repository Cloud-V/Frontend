    ...
    render() {
        ...
        let onSubmit = async (firstFormData) => {
            ...
            const modules = _.map(
                this.props.files
                    .get("data")
                    .filter((el) => el.get("parent") === nodeId)
                    .toJS(),
                "title"
            );
            this.setState({
        ...
        if (step === 0) {
            ...
            let defaultFileName =
                this.state.firstFormData.get("verilogFile").title;
            ...
            return (
                <NewTestbenchSecondForm
                    ...
                    onSubmit={(secondFormData) => {
                        const merged =
                            this.state.firstFormData.merge(secondFormData);
...
export default connect(mapStateToProps, mapDispatchToProps)(TestbenchWizard);
