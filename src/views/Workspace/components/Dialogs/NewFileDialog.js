...
const createFileForm = ({ fieldName, fileType, fileExtension }) => props => (
	<NewFileForm
		{...props}
										...
										>
											{_.map(cat.types || [], type => (
												<NavItem key={type.id}>
													...
													>
														<span
															className={`file-icon dialog-icon ${
																type.icon
															}`}
														/>
														{type.name}
	...
	render() {
		...
		return (
			<Modal
				...
				backdrop="static"
				className={`dialog theme-wrapper workspace-dialog new-file-dialog theme-${
					this.props.theme
				}`}
				centered={true}
				onClosed={() => {
				...
				<ModalBody className="modal-form d-flex flex-column">
					{(() => {
						if (this.props.newFileStep) {
							...
							const NextComponent = this.state.currentType
								.component;
							return (
								<NextComponent
