		...
		if (stats.get('status') === 'loading' || (stats.get('status') === '')) {
			...
			return (<div className="app">
					...
					<div className="flex-1 w-100 d-flex">
						<Alert className="h-10 flex-1" color="danger">{stats.get('error')}</Alert>
					</div>
					<Footer />
				</div>);
		}
				...
				<Container className="tools flex-1 w-100 h-100 pl-0 pr-0 d-flex flex-column justify-content-center align-items-center" fluid>
					<Row className="w-100">
						<Col className="center-all" md="12">
							<h1 className="mb-4 pb-0 mt-3 font-weight-bold text-center">Admin Statistics</h1>
						</Col>
					</Row>
								...
								<Table responsive hover>
									...
									<tbody>
										...
										<tr>
											...
											<td>{statsJson.testbenchEntries}</td>
										</tr>
										<tr>
											<td>No. of Simulations</td>
											<td>{statsJson.simulationEntries}</td>
										</tr>
									</tbody>
								</Table>
							</Col>
