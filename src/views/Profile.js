...
import { Alert, Button, Container, Col, Card, CardBody } from "reactstrap";
...
class Profile extends Component {
    constructor(props) {
        ...
        this.isMe =
            this.props.user.get("data").get("username") === this.ownerName;
    }
    ...
    render() {
        ...
        const displayName =
            ...
            (profileData.username.length ? "@" + profileData.username : "");
        const about = profileData.about;

        console.log("Profile Data:", profileData);
        const avatarURL = getGravatarURL(
            profileData.gravatarEmail
                ? profileData.gravatarEmail
                : profileData.email
        );
                    ...
                    <div className="w-100 auth-box">
                        <Card className="w-100">
                            <CardBody className="profile-card w-100 d-flex flex-column justify-content-start align-items-center position-relative">
                                {this.isMe && (
                                    <div className="w-100 d-flex justify-content-end edit-profile-wrapper">
                                        <Button
                                            title="Edit Profile"
                                            tag={Link}
                                            to={"/edit"}
                                            className="icon-btn mr-0 edit-profile-btn"
                                        >
                                ...
                                <div className="profile-about-title w-100 d-flex justify-content-start mb-3">
                                    About {displayName}:
                                </div>
                                <div className="profile-about w-100 d-flex justify-content-start mb-3">
                                    {about.length ? (
                                        about
                                    ) : (
                                        <i>No about section provided.</i>
                                    )}
                                </div>
                                <Link
                                    to={`repositories/${this.ownerName}`}
                                    className="profile-repositories-link plain-link black d-flex w-100 justify-content-start"
                                >
                                    Repositories
                                </Link>
                            </CardBody>
